...
DEFAULT_TRUTH_TABLE_PATH = Path(__file__).resolve().parent.parent / "testdata" / "cn_stock_truth_table.json"
...
def main() -> int:
    if len(sys.argv) > 2:
        raise SystemExit("usage: python verify_cn_stock_truth_table_with_pandas_market_calendars.py [truth_table_path]")
    ...
    mismatches = compare_truth_table(payload, "XSHG")
